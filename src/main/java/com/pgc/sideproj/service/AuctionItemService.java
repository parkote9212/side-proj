...
public class AuctionItemService {
     ...
     */
    public PageResponseDTO<AuctionItemSummaryDTO> getItems(String keyword,
                                                           String region,
                                                           Long priceFrom,  // 💡 파라미터 추가
                                                           Long priceTo,
                                                           String dateFrom,
                                                           String dateTo,
                                                           int page,
        ...
        int totalCount = auctionItemMapper.countItems(keyword, region, priceFrom, priceTo, dateFrom, dateTo);

        // 3. DB에서 데이터 목록 조회 (FTS 검색어, region, 페이지네이션 포함)
        List<AuctionItemSummaryDTO> items = auctionItemMapper.findItems(keyword, region, priceFrom, priceTo, dateFrom, dateTo, offset, size );
